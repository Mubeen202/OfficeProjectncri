...
@Injectable()
export class CandidateEducation implements Resolve<any> {
  // Public
  public emails: Email[];
  ...
  getEmailRecords(value): Promise<any[]> {
    return new Promise((resolve, reject) => {
      this._httpClient
        .post(`${environment.apiUrl}/gettingCandidateEducation`, value)
  ...
  getCandidateEducationRecords(value) {
    const apiUrl = `${environment.apiUrl}/candidateEducation`;
    return new Promise<void>((resolve, reject) => {
      this._httpClient.post(apiUrl, value).subscribe((response: any) => {
        this.onUnreadInboxCountChanged.next(response);
        resolve();
