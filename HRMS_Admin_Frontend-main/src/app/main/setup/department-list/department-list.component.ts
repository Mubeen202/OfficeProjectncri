...
export class DepartmentListComponent implements OnInit {
   ...
   * @param {DepartmentEditService} _userListService
   * @param {CoreSidebarService} _coreSidebarService
   * @param {DatatablesService} _dataTablesService
   */
  constructor(
    private _userListService: DepartmentEditService,
 ...
 /**
  ...
  ngOnInit() {
    ...
    this._coreConfigService.config.pipe(takeUntil(this._unsubscribeAll)).subscribe(config => {
      ...
      if (config.layout.animation === 'zoomIn') {
        setTimeout(() => {
          this._userListService.onUserEditChanged.pipe(takeUntil(this._unsubscribeAll)).subscribe(response => {
            this.rows = response;
  ...
  editDepartment(row:any){
    if(row)
   { 
    localStorage.setItem('departmentId', JSON.stringify(row))
  }
  }
  ...
  getTenantList() {
    this._userListService.gettAllDepartments().then((data: any) => {
...
filteredTenant() {
  this._userListService.searchDepartmentList(this.name).then((data: any) => {
