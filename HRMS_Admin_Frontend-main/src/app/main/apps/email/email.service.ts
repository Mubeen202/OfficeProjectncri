...
export class EmailService implements Resolve<any> {
  ...
  public EmailListing: BehaviorSubject<any>;
  ...
  constructor(private _httpClient: HttpClient, private router: Router) {
    ...
    this.EmailListing = new BehaviorSubject([]);
  ...
  sendEmail(value) {
    const apiUrl = `${environment.apiUrl}/send`;
    return new Promise<void>((resolve, reject) => {
      this._httpClient.post(apiUrl, value).subscribe((response: any) => {
        this.onUnreadInboxCountChanged.next(response);
        resolve();
      }, reject);
    });
  }
  ...
  createNotes(value) {
    const apiUrl = `${environment.apiUrl}/recruiterNotes`;
    return new Promise<void>((resolve, reject) => {
      this._httpClient.post(apiUrl, value).subscribe((response: any) => {
        this.onUnreadInboxCountChanged.next(response);
        resolve();
  ...
  getCandidateRecords(value): Promise<any[]> {
    return new Promise((resolve, reject) => {
      this._httpClient
        .post(`${environment.apiUrl}/gettingRecruiterNotes`, value)
        .subscribe((response: any) => {
          this.rows = response;
          this.onUnreadInboxCountChanged.next(this.rows);
          resolve(this.rows);
        }, reject);
    });
  }

  getEmailRecords(value): Promise<any[]> {
    return new Promise((resolve, reject) => {
      this._httpClient
        .post(`${environment.apiUrl}/getEmailDetails`, value)
        .subscribe((response: any) => {
          this.rows = response;
          this.onUnreadInboxCountChanged.next(this.rows);
          resolve(this.rows);
  ...
  getAllNotes(value) {
    const apiUrl = `${environment.apiUrl}/gettingRecruiterNotes`;
    return new Promise<void>((resolve, reject) => {
      this._httpClient.post(apiUrl, value).subscribe((response: any) => {
        this.onUnreadInboxCountChanged.next(response);
        resolve();
  ...
  openEmailDetails(id): void {
    ...
    this.getEmailRecords(value).then((data: any) => {
      data.result.items.map((item) => {
        item.map((item) => {
          if (item.id === id) {
            this.openedEmail.push(item);
          }
        });
      });
    });
